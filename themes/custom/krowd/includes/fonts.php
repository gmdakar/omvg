...
function krowd_render_option_font(){
    $fonts = krowd_fonts();
    $output = '';
...
function krowd_typography_font_styles($option, $selectors) {
    $output = $selectors . ' {';
...
function krowd_typography_enqueue_google_font($font) {
 
    if($font && $font != "---"){
        if(array_search($font, array_keys(krowd_fonts())) > 17){
...
function krowd_links_typography_font($json){
    $links_fonts = '';
    $customize = (array)json_decode($json, true);
    if(isset($customize['font_family_primary']) && $customize['font_family_primary']){
        $links_fonts .= krowd_typography_enqueue_google_font($customize['font_family_primary']);
    }
    if(isset($customize['font_family_second']) && $customize['font_family_second']){
        $links_fonts .= krowd_typography_enqueue_google_font($customize['font_family_second']);
    }
    return $links_fonts;
}

function krowd_options_patterns(){
    $output = '';
    $file_path = drupal_get_path('theme', 'krowd');
